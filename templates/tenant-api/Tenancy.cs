...
/// <summary>The two seeded demo tenants — pass one as the <c>X-Tenant-Id</c> header.</summary>
...
// ── Feature: list the current tenant's widgets (auto-filtered by the multi-tenant query filter) ──
...
// Registers a new tenant in the registry with its own connection string and provisions its database on
// the fly — no code change, no redeploy. In production the connection string would be issued by your
// infrastructure / secret store rather than derived from a fresh GUID.
    ...
    IGuidGenerator guids) : ICommandHandler<OnboardTenantCommand, Result<OnboardedTenant>>
{
    public async Task<Result<OnboardedTenant>> HandleAsync(OnboardTenantCommand command, CancellationToken ct)
    {
        ...
        // 1) Register the tenant via the management API (persists to the SliceTenants registry).
