...
// A standalone migration job: composes the SAME module graph as the API (TenantModule), then applies EF
// migrations to the host database + every tenant in the SliceTenants registry — and exits. Run this as a
// deploy/CI step (or a Kubernetes Job) before rolling out the app, with the app started using
...
// Optional real single-runner coordination across concurrent migration jobs: set ConnectionStrings:Redis
// (env: ConnectionStrings__Redis). Registered AFTER AddSliceModules so it overrides the no-op default.
